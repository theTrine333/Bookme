...
export const DownloadsCard = ({
  ...
  const theme = useColorScheme() ?? "light";
  const deleter = useBooks();
  const isToBeDeleted = deleter.selectedBooks.some(
    (deletes) => deletes === Title
  );
  return (
    <Pressable
      onLongPress={() => {
        deleter.toggleBookSelection(Title);
      }}
      onPress={async () => {
        if (deleter.selectedBooks.length > 0) {
          deleter.toggleBookSelection(Title);
          return;
        }
        router.push({
    ...
    >
      {isToBeDeleted ? (
        <ImageBackground
          ...
          imageStyle={{ borderBottomLeftRadius: 12, borderTopLeftRadius: 12 }}
          placeholder={require("@/assets/images/pdf.png")}
          contentFit="cover"
        >
          <ThemedView style={Styles.headerButtons} hitSlop={20}>
